...
BuildImportTable(PLOADEDMODULE pLoadedModule)
{
	...
	__try
	{
		...
		for (; importDesc && importDesc->Name; importDesc++)
		{
			...
			tmp = (HINSTANCE*)realloc(pLoadedModule->pModules, ((SIZE_T)pLoadedModule->nModules + 1) * (sizeof(HINSTANCE)));
			if (NULL == tmp)
			{
...
FARPROC _GetProcAddress(__in_opt PLOADEDMODULE pModule, __in_z_opt LPCSTR FuncName)
{
	...
	return (FARPROC)(PVOID)(pModule->pCodeBase + (*(DWORD*)(pModule->pCodeBase + exports->AddressOfFunctions + ((SIZE_T)idx * 4))));
}
...
PLOADEDMODULE _LoadModuleFromInstance(__in HINSTANCE hInstance)
{
	...
	pLoadedModule->pCodeBase = (PBYTE)(PVOID)pNtHeaders->OptionalHeader.ImageBase;
...
__checkReturn __success(return != NULL) PLOADEDMODULE LoadModuleFromMemory(__in_bcount(dwSize) PVOID lpData, __in DWORD dwSize)
{
	...
	for (i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++, pSection++)
	{
		...
		if (pSection->SizeOfRawData == 0)
		{
			...
			endOfSection = (SIZE_T)pSection->VirtualAddress + pNtHeaders->OptionalHeader.SectionAlignment;
		}
		else
		{
	...
	pImage = (PUCHAR)VirtualAlloc((PVOID)pNtHeaders->OptionalHeader.ImageBase,
		ImageSize,
...
static PIMAGE_RESOURCE_DIRECTORY_ENTRY SearchResourceEntry(
	__in void* root,
	__in PIMAGE_RESOURCE_DIRECTORY resources,
	__in_opt LPCTSTR key)
{
...
__checkReturn __success(return == TRUE) BOOL IsPEHeaderValid(__in PVOID lpData, __in DWORD dwSize)
{
	...
	if (pDosHeader->e_magic == IMAGE_DOS_SIGNATURE)
	{
		...
		{
#endif
			// log: Incorrect architecture type: %x
			return FALSE;
		}
